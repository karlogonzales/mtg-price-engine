...
class PriceEngine:
    ...
    async def search_401games(self, session, card_name, quantity):
        try:
            url = f"https://api.fastsimon.com/full_text_search?request_source=v-next&src=v-next&UUID=d3cae9c0-9d9b-4fe3-ad81-873270df14b5&store_id=17041809&q={quote(card_name)}&narrow=[[%22In+Stock%22,%22True%22]]&page_num=1&products_per_page=40"
    ...
    async def search_hobbiesville(self, session, card_name, quantity):
        try:
            url = (
                "https://stable.storepass.co/saas/search"
                "?store_id=cGCfZUZiX0"
                "&mongo=true"
            ...
            print(f"Hobbiesville error ({card_name}): {e}")
            return None
    ...
    async def search_all_stores(self, session, card_name, quantity, store_sem):
        async with store_sem:
            tasks = [
                ...
                self.search_hobbiesville(session, card_name, quantity),
            ]
            ...
            async def run_card(i, card):
                async with card_sem:
                    ...
                    return card["name"], {
                        ...
                        "found": bool(
                            results
                        ),  # ✅ True if any store returned a result
                    }

            tasks = [run_card(i, card) for i, card in enumerate(card_list)]
    ...
    def display_results(self, all_results):
        ...
        for card_name, data in all_results.items():
            ...
            for result in results:
                ...
                print(
                    f"  {result['store']:20} | ${result['price']:6.2f} each | Total: ${result['total_cost']:7.2f} | {status}"
                )
        ...
        for card_name, data in all_results.items():
            for result in data["results"]:
                if result["in_stock"]:
                    ...
                    store_totals[store]["cards"].append(
                        f"{card_name} (${result['price']:.2f} x {data['quantity']})"
                    )
